...
const bundle = {
  ...
  rules: {
    ...
    rules: [...managedRules.rules, { name: 'netease-music', exit: '', dns_pool: 'cn_netease', domain_suffix: ['music.163.com', 'music.126.net', 'iplay.163.com', 'look.163.com', 'y.163.com'] }, { name: 'china-unicom-app', exit: '', dns_pool: 'cn_unicom', domain_suffix: ['10010.com', 'chinaunicom.cn', 'chinaunicom.com'] }, { name: 'openai-and-related-services', exit: 'tokyo-shadowsocks-production-long-name', dns_pool: '', domain_suffix: ['openai.com', 'chatgpt.com', 'very-long-service-domain.example.net'] }],
  },
}
...
async function mockAPI(page: Page, applyHandler?: (route: Route) => Promise<unknown>) {
  await page.route('**/api/v1/**', async route => {
    ...
    if (path === '/api/v1/diagnostics/run') return json({ checked_at: '2026-07-12T12:00:00Z', dns: [
      ...
      { pool: 'cn_unicom', upstream: '180.76.76.76', protocol: 'udp', status: 'ok', latency_ms: 18.2, answers: ['39.156.8.67'] },
    ], exits: [
...
test('SS exit name supports sequential editing while local SOCKS stays internal', async ({ page }) => {
	...
	await expect(exits.locator('tbody tr')).toHaveCount(2)
	const pending = page.locator('form').filter({ has: page.getByRole('heading', { name: '新建 SS 出口', exact: true }) })
	const name = pending.getByLabel('新 SS 出口名称')
	await expect(name).toHaveValue('ss2')
	await name.fill('')
	await name.pressSequentially('osaka-ss')
	await expect(name).toHaveValue('osaka-ss')
	await expect(name).toBeFocused()
	const complexExitName = '🇯🇵 Tokyo SS'
	await name.fill(complexExitName)
	await expect(name).toHaveValue(complexExitName)
	await pending.getByLabel('新 SS 出口服务器', { exact: true }).fill('osaka.example.net')
	await pending.getByLabel('新 SS 出口服务器端口', { exact: true }).fill('8443')
	await pending.getByLabel('新 SS 出口加密方法', { exact: true }).selectOption('2022-blake3-aes-256-gcm')
	const details = page.locator('section').filter({ has: page.getByRole('heading', { name: complexExitName, exact: true }) })
	const method = details.getByLabel('加密方法')
	const pendingPassword = pending.getByLabel('新 SS 出口密码')
	const identityKey = Buffer.from('s'.repeat(32)).toString('base64')
	const userKey = Buffer.from('u'.repeat(32)).toString('base64')
	const passwordChain = `${identityKey}:${userKey}`
	await pendingPassword.fill(passwordChain)
	await page.getByRole('button', { name: '应用', exact: true }).click()
	await expect(page.getByText('请先确认添加或取消正在新建的项目')).toBeVisible()
	await expect(exits.locator('tbody tr')).toHaveCount(2)
	await pending.getByRole('button', { name: '确认添加', exact: true }).click()
	await expect(exits.locator('tbody tr')).toHaveCount(3)
	await expect(method).toHaveValue('2022-blake3-aes-256-gcm')
	...
	await expect(details).toContainText('每段为 Base64 编码的 32 字节密钥（通常 44 个字符）')
	const password = details.getByLabel('密码')
	const existingPassword = page.locator('section').filter({ has: page.getByRole('heading', { name: 'tokyo-shadowsocks-production-long-name', exact: true }) }).getByLabel('密码')
	await method.selectOption('2022-blake3-aes-256-gcm')
	await expect(details).toContainText('支持 iPSK[:...]:uPSK，每段为 Base64 编码的 32 字节密钥（通常 44 个字符）')
	await password.fill(passwordChain)
	await expect(password).toHaveAttribute('type', 'password')
	await details.getByTitle('显示密码').click()
	await expect(password).toHaveAttribute('type', 'text')
	await expect(password).toHaveValue(passwordChain)
	await expect(existingPassword).toHaveAttribute('type', 'password')
	await details.getByTitle('隐藏密码').click()
	await expect(password).toHaveAttribute('type', 'password')
	await method.selectOption('2022-blake3-chacha20-poly1305')
	await expect(details).toContainText('仅支持单段 Base64 编码的 32 字节密钥')
	await method.selectOption('2022-blake3-aes-256-gcm')
	await expect(password).toHaveValue(passwordChain)
	await page.getByRole('button', { name: '应用', exact: true }).click()
	await expect(page.getByText('配置已应用，共 10023 条规则')).toBeVisible()
	const added = submitted?.config.exits.find(exit => exit.name === complexExitName)
	expect(added).toEqual(expect.objectContaining({ method: '2022-blake3-aes-256-gcm', password: passwordChain, listen_address: '127.0.0.1', listen_port: 1081 }))
})

test('new rule and remote import require confirmation before applying', async ({ page }) => {
	let submitted: typeof bundle | undefined
	await mockAPI(page, async route => {
		submitted = route.request().postDataJSON() as typeof bundle
		return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ id: 'test', status: 'succeeded', changed: true, revision_id: 8, rule_count: 10023, warnings: null }) })
	})
	await page.goto('/')
	...
	await expect(page.getByLabel('新规则名称')).toBeFocused()
	await page.getByRole('button', { name: 'DNS 与网络', exact: true }).filter({ visible: true }).click()
	await expect(page.getByRole('heading', { name: '规则', exact: true })).toBeVisible()
	await expect(page.getByText('请先确认添加或取消正在新建的项目')).toBeVisible()
	await page.getByRole('button', { name: '规则', exact: true }).filter({ visible: true }).first().click()
	const complexRuleName = '🇯🇵 Tokyo Rule'
	await page.getByLabel('新规则名称').fill(complexRuleName)
	await page.getByLabel('新规则域名').fill('manual-confirmed.invalid')
	await page.getByRole('button', { name: '确认添加', exact: true }).click()
	await expect(localRules.getByPlaceholder('名称').last()).toHaveValue(complexRuleName)

	await page.getByRole('button', { name: '导入', exact: true }).click()
	const complexImportName = '远程规则 🚀'
	...
	await expect(page.getByLabel('新导入名称')).toHaveCount(0)
	await page.getByRole('button', { name: '导入', exact: true }).click()
	await page.getByLabel('新导入名称').fill(complexImportName)
	await page.getByLabel('新导入地址').fill('https://rules.example.test/remote.json')
    ...
    await expect(page.getByRole('button', { name: /历史/ })).toBeHidden()
      ...
      const button = page.getByRole('button', { name: nav, exact: true }).filter({ visible: true })
			...
			if (heading === '规则') {
				...
				await expect(custom.getByPlaceholder('名称').nth(1)).toHaveValue('china-unicom-app')
				await expect(custom.getByPlaceholder('名称').nth(2)).toHaveValue('openai-and-related-services')
...
test('custom DNS pool rename updates rules and optional default pool deletion cascades', async ({ page }) => {
	await mockAPI(page)
	await page.goto('/')
	await page.getByRole('button', { name: 'DNS 与网络', exact: true }).filter({ visible: true }).click()
	const pools = page.locator('section').filter({ has: page.getByRole('heading', { name: '自定义 DNS 池' }) })
	const neteasePool = pools.getByPlaceholder('custom_pool').first().locator('..').locator('..')
	const name = neteasePool.getByPlaceholder('custom_pool')
	...
	await neteasePool.getByTitle('删除 DNS 池').click()
	await expect(pools.getByPlaceholder('custom_pool')).toHaveCount(1)
	await page.getByRole('button', { name: '规则', exact: true }).filter({ visible: true }).first().click()
	await expect(localRules.getByPlaceholder('名称')).toHaveCount(2)
	await expect(localRules.getByPlaceholder('名称').first()).toHaveValue('china-unicom-app')
...
test('manual rule apply reconnects with the same operation ID', async ({ page }) => {
	...
	await page.getByRole('button', { name: '新建规则', exact: true }).click()
	const localRules = page.locator('section').filter({ has: page.getByRole('heading', { name: '自定义本地规则' }) })
	await localRules.getByLabel('新规则名称').fill('manual-smoke')
	await localRules.getByLabel('新规则域名').fill('manual-smoke.invalid')
	await localRules.getByRole('button', { name: '确认添加', exact: true }).click()
	await page.getByRole('button', { name: '应用', exact: true }).click()
