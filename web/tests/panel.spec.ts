...
test('new rule and remote import require confirmation before applying', async ({ page }) => {
	...
	await expect(page.getByText('请先确认添加或取消正在新建的项目')).toBeVisible()
	await page.getByRole('button', { name: '规则', exact: true }).filter({ visible: true }).first().click()
	await page.getByLabel('新规则名称').fill('manual-confirmed')
	await page.getByLabel('新规则域名').fill('manual-confirmed.invalid')
	await page.getByRole('button', { name: '确认添加', exact: true }).click()
	await expect(localRules.getByPlaceholder('名称').last()).toHaveValue('manual-confirmed')

	await page.getByRole('button', { name: '导入', exact: true }).click()
	await page.getByLabel('新导入名称').fill('remote-confirmed')
	...
	await page.getByRole('button', { name: '应用', exact: true }).click()
	await expect(page.getByText('配置已应用，共 10023 条规则')).toBeVisible()
	await expect.poll(() => submitted?.rules.rules).toContainEqual(expect.objectContaining({ name: 'manual-confirmed', domain_suffix: ['manual-confirmed.invalid'] }))
	await expect.poll(() => submitted?.rules.imports).toContainEqual(expect.objectContaining({ name: 'remote-confirmed', url: 'https://rules.example.test/remote.json' }))
})
