...
func TestNFTablesRedirectOnlyInternalCIDR(t *testing.T) {
	...
	if strings.Contains(out, `udp dport 443 counter redirect to :18443`) {
		t.Fatalf("UDP/443 should not redirect when quic_policy=reject:\n%s", out)
	}
	if strings.Contains(out, `tcp dport 8080 counter redirect to :18081`) {
		t.Fatalf("default Speedtest TCP proxy redirect must not render:\n%s", out)
	}
	...
	if strings.Contains(out, `udp dport 3478 counter redirect to :13478`) {
		t.Fatalf("default STUN UDP proxy redirect must not render:\n%s", out)
	}
	...
	if !strings.Contains(out, `ip saddr != 10.0.0.0/24 tcp dport { 1053, 1853, 18080, 18082, 18443 } reject with tcp reset`) {
...
func TestNFTablesCanProxyQUICWhenEnabled(t *testing.T) {
	cfg := testConfig()
