...
func TestConfigRendersAddressRules(t *testing.T) {
	...
	for _, want := range []string{
		...
		"server https://cloudflare-dns.com/dns-query -group overseas_public -exclude-default-group",
...
func TestConfigRendersCustomDNSPool(t *testing.T) {
	...
	for _, want := range []string{
		...
		"server 52.80.66.66 -group cn_netease -exclude-default-group",
		"server 210.2.4.8 -group cn_netease -exclude-default-group",
